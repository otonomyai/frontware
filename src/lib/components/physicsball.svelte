<script>
    ...
    onMount(() => {
      ...
      for (let i = 0; i < ballcount; i++) {
        const ball = Bodies.circle(Math.random() * canvasWidth, Math.random() * canvasHeight, ballsize, {
          restitution: 0.1,
          render: {
            fillStyle: "#1E375C"
          },
      ...
      const ground = Bodies.rectangle(canvasWidth / 2, canvasHeight - 45, canvasWidth, 50, {
        isStatic: true,
      ...
      const rightWall = Bodies.rectangle(canvasWidth + 5, canvasHeight / 2, 50, canvasHeight, {
        isStatic: true,
        render: { visible: false }
      });
      ...
      engine.world.bodies.forEach((body, index) => {
        if (body.label === 'Circle Body') {
          console.log(`Ball ${index}: Position (${body.position.x}, ${body.position.y})`);
        }
      });
    });
  </script>
